...
def draw_poly(cam_xyz, points):
    ...
    if cube_ang < 0:
        cube_ang += 2*np.pi
    return corners, cube_ang, cube_angxz, dist_xz

def draw_cube(cam_xyz, tops, bots, side1s, side2s, side3s, side4s):
    #draw cube
...
class cube:
    ...
    whl = [50,50,50]
    def run(self):
        tops, bots, sides1, sides2, sides3, sides4 = stationary_cube(self.xyz, self.whl)
        polys = draw_cube(self.xyz, tops, bots, sides1, sides2, sides3, sides4)
        print('poopys')

#find cube points
top, bot, side1, side2, side3, side4 = stationary_cube(box_xyz, box_whl)
top2, bot2, side12, side22, side32, side42 = stationary_cube(cube_xyz, cube_whl)
...
while run:
    ...
    #draw the first cube
    if dist1 > dist2:
        dist1 = draw_cube(camera_xyz, top, bot, side1, side2, side3, side4)
        dist2 = draw_cube(camera_xyz, top2, bot2, side12, side22, side32, side42)
    else:
        dist2 = draw_cube(camera_xyz, top2, bot2, side12, side22, side32, side42)
        dist1 = draw_cube(camera_xyz, top, bot, side1, side2, side3, side4)
    ...
    if keys[pygame.K_g]:
        look_toggle += 1
    if look_toggle%2:
        camera_ang -= mouse_dif[0]/(screen_width*3)
